...
static void char2bit(unsigned char datachar,int *databit)
{
	int i;
	for(i=0;i<8;i++) databit[i]=(datachar>>(7-i))&0x01;
	return;
}
static void chartobit(unsigned char *sour,int data[])
{
	int i;
	for(i=0;i<8;i++){
...
static void boxer(int data01[48],int bits[32])
{
	int i=0;
	int row,line;
	int result[8];
...
static void expand(int data01[],int data02[])
{
	int i;
	for(i=0;i<48;i++){
...
static void bitxor(int data01[],int data02[],int len)
{
	int i;
	for(i=0;i<len;i++){
...
static void bittochar(int data01[],unsigned char *data)
{
	int i;
	for(i=0;i<8;i++){
...
static void fpc1(unsigned char *key,int c[],int d[])
{
	int temp[64];
	int data01[56];
	int i;
...
static void fkey(unsigned char *key,int sk[16][48])
{
	int c[28];
	int d[28];
	int temp[56];
...
static  void protocol_des(unsigned char *key,unsigned char *sour,unsigned char *dest,int m)
{
	int i,j;
	int data01[64];
	int l[32];
	...
	//printf("protocol_des start key = %s\n",key);
	fkey(key,k);
...
//Triple DES
int protocol_tripledes(unsigned char *key,unsigned char *src,unsigned char *dest,int m)
{
	...
	protocol_des(key0,src0,data,flag);
	//printf("protocol_des 1\n");
...
static int creat_mac(unsigned char KeyLen,unsigned char *Key, int DataLen,
...
//	StdCharsToHexChars(InitData, TempData, 8);
	...
	mode=1; //Encryption
	if(0x08==KeyLen)
		protocol_des(&Key[0],TempData,InitData,mode);
	else
		protocol_tripledes(&Key[0],TempData,InitData,mode);
	...
	mode=1; //Encryption
	if(0x08==KeyLen)
		protocol_des(&Key[0],InitData,InitData,mode);
	else
		protocol_tripledes(&Key[0],TempData,InitData,mode);
