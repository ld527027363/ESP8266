...
static void Protocol_Encrypt(PProtocol_Layer_ST layer,Pchar data,unsigned short datalen)
{
    ...
    while(datalen)
    {
        protocol_tripledes((unsigned char*)layer->ParamList[PP_SESSIONKEY],(unsigned char*)data,(unsigned char*)data,ENCRYPT);
        data += 8;
        datalen -= 8;
    }
}
...
static void Protocol_Decrypt(PProtocol_Layer_ST layer,Pchar data,unsigned short datalen)
{
   ...
   // printf("datalen:%d,DECRYPT_Data:%s\r\n",datalen,data);
    ...
    while(bufferlen)
    {
        //protocol_tripledes((unsigned char*)layer->ParamList[PP_SESSIONKEY],(unsigned char*)data,(unsigned char*)data,DECRYPT);
        protocol_tripledes((unsigned char*)layer->ParamList[PP_SESSIONKEY],(unsigned char*)buffer,(unsigned char*)buffer,DECRYPT);
        buffer += 8;
        bufferlen -= 8;
    }
